...
def test_template_card_is_valid(card_template: dict[str, object]) -> None:
...
def test_card_is_frozen(card_template: dict[str, object]) -> None:
    card = ModelCard.model_validate(card_template)
...
def test_bad_weights_are_rejected(card_template: dict[str, object], field: str, value: str) -> None:
    weights = dict(card_template["weights"])  # type: ignore[arg-type]
    weights[field] = value
...
def test_unknown_field_is_rejected(card_template: dict[str, object]) -> None:
    with pytest.raises(ValidationError):
        ModelCard.model_validate({**card_template, "trainig_dataset": "typo"})


def test_unknown_task_is_rejected(card_template: dict[str, object]) -> None:
    with pytest.raises(ValidationError):
        ModelCard.model_validate({**card_template, "task": "telekinesis"})


def test_normalization_length_must_match_channels(card_template: dict[str, object]) -> None:
    payload = {
        **card_template,
...
def test_roundtrip_through_yaml(card_template: dict[str, object]) -> None:
    card = ModelCard.model_validate(card_template)
